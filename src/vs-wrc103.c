...
void pass_cross(void){
	...
	while (1)
	{
	 ...
	 right = ADRead(1);
	 delta = left-down_MID;
	 alpha = right-down_MID;
	float left_pid = P*delta + D*(delta-pastAD0);
	float right_pid = P*alpha +D*(delta-pastAD1);
	pastAD0 = delta;
	pastAD1 = alpha;
	
	if (left_pid+right_pid<=5000){
		counter +=1;
			if (counter==1000)
			{	
				motor_stop();
	...
	{	
		counter = 0;
		Mtr_Run_lv(right_SPEED*(left_pid+right_pid),left_SPEED*(right_pid+left_pid),0,0,0,0);
	}
...
void move(void)
{
	...
	while(1){
		//数値更新
		short down_left=ADRead(0);
		short down_right=ADRead(1);
		/*  pid*/
		short delta = left_MID-down_left;
		short alpha = right_MID-down_right;
		float left_pid = P*delta + D*(delta-pastAD0);
		float right_pid = P*alpha +D*(alpha-pastAD1);
		//昔の数値情報更新
		pastAD0 = delta;
		pastAD1 = alpha;
		if (left_pid<= -2000)
		...
		{
		Mtr_Run_lv(right_SPEED*(SPEED*right_pid),left_SPEED*(SPEED*left_pid),0,0,0,0);
		counter =0;
		}
